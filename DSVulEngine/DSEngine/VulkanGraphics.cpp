...
// Initialize Vulkan
void VulkanGraphics::Initialize(GLFWwindow* window)
{
    m_Window = window;
    glfwSetFramebufferSizeCallback(m_Window, FramebufferResizeCallback);
    glfwSetWindowUserPointer(m_Window, this);

    // Create Instance of Vulkan First
    CreateInstance(&m_Instance);

    // Check if we want to validate API Calls
    ...
    // Pick the Graphics Card to use
    PickPhysicalDevice();
    // Create a Logical Device to communicate with the chosen Graphics Card
    CreateLogicalDevice();
    // Create the Chain that will queue the images to display on the surface
    CreateSwapChain();
...
void VulkanGraphics::DrawFrame()
{
    ...
    // We want to check if the image is valid with the current swapchain/
    result = vkQueuePresentKHR(m_PresentQueue, &presentInfo);

    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || m_FramebufferResized)
    {
        m_FramebufferResized = false;
        // Recreate swap chain if image out of date
        RecreateSwapChain();
    }
...
static void FramebufferResizeCallback(GLFWwindow* window, int width, int height) {
    auto app = reinterpret_cast<VulkanGraphics
